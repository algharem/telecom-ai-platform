...
class PrometheusDataProvider(DataProvider):
    ...
    def derive_kpis(self, metrics):
        prb_usage = self.derivation["prb_base"] + metrics["registered_ues"] * self.derivation["prb_per_ue"]
        throughput = metrics["pdu_sessions"] * self.derivation["mbps_per_session"]
        latency = self.derivation["latency_base_ms"] + (metrics["auth_fails"] / metrics["auth_requests"]) * self.derivation["latency_penalty_ms"]
        packet_loss = (metrics["auth_fails"] / metrics["auth_requests"]) * 100 if metrics["auth_requests"] > 0 else 0.0
        return {
            "prb_usage": prb_usage,
    ...
    def fetch_metrics(self):
        metrics = {
            "registered_ues": self.prom_client.query_metric("fivegs_amffunction_rm_registeredsubnbr"),
            "pdu_sessions": self.prom_client.query_metric("fivegs_smffunction_sm_sessionnbr"),
            "auth_requests": self.prom_client.query_rate_metric("fivegs_amffunction_amf_authreq"),
            "auth_fails": self.prom_client.query_rate_metric("fivegs_amffunction_amf_authfail"),
        }
        return metrics
